...
window.healthRecordData = [
  {
    ...
    meals: [
      {
        label: "저녁",
        items: ["페리카나 후라이드·양념 반반 치킨 (섭취량 미확인)", "일반 콜라 1잔 (제품·용량 미확인)"]
      }
    ],
    nutritionEstimate: {
      calories: "약 1,000~1,900 kcal (기록된 저녁만)",
      protein: "약 55~125g (기록된 저녁만)",
      freeSugar: "약 40~80g (기록된 저녁만)",
      nutrients: {
        energy: { min: 1000, max: 1900, unit: "kcal" },
        protein: { min: 55, max: 125, unit: "g" },
        carbohydrate: { min: 55, max: 145, unit: "g" },
        totalFat: { min: 50, max: 120, unit: "g" },
        saturatedFat: { min: 12, max: 32, unit: "g" },
        freeSugar: { min: 40, max: 80, unit: "g" },
        fiber: { min: 1, max: 4, unit: "g" },
        sodium: { min: 1800, max: 4000, unit: "mg" }
      },
      assumptions: "페리카나 후라이드와 양념치킨을 섞어 가식부 약 300~550g 먹은 것으로 가정했다. 페리카나 공식 메뉴의 한 마리 조리 전 원료육 중량은 10호(951~1,050g) 이상이지만 뼈·조리 손실과 실제 섭취량을 알 수 없어 넓은 범위를 사용했다. 콜라는 일반 제품 약 250ml 한 잔으로 가정해 약 112kcal·당류 27g을 반영했다. 제로콜라였다면 열량과 유리당 추정치는 크게 낮아진다. 오늘 다른 끼니는 알려지지 않아 이 값은 하루 총섭취량이 아니라 현재 기록된 저녁만의 추정치다.",
      target: "최근 측정 체중 65.8kg 기준 단백질 우선 목표는 약 105g/일이다. 다만 오늘은 저녁만 기록되어 일일 충족·초과를 확정하지 않는다. 에너지·탄수화물·지방·식이섬유는 2025 한국인 영양소 섭취기준의 남성 30~49세 참고치와, 포화지방·유리당·나트륨은 WHO 상한을 비교용으로만 표시한다.",
      sources: [
        { label: "페리카나 양념치킨 공식 메뉴", href: "https://www.pelicana.co.kr/menu/detail?goodsNo=1000100299" },
        { label: "페리카나 후라이드 공식 메뉴", href: "https://www.pelicana.co.kr/menu/detail?goodsNo=1000100302" },
        { label: "한국소비자원 프랜차이즈 치킨 비교", href: "https://www.kca.go.kr/home/sub.do?menukey=4004&mode=view&no=1001886181" },
        { label: "코카콜라 250ml 영양정보", href: "https://www.costco.co.kr/Foods/Beverages/SoftConcentrated-Drinks/Coca-Cola-Original-250ml-x-30/p/626670" }
      ]
    },
    condition: "저녁 식사만 기록 · 운동 여부 미기록 · 체성분 미측정 · 치킨과 콜라 섭취량 미확인",
    advice: {
      title: "현재 판정: 저녁은 단백질을 포함하지만 지방·유리당·나트륨이 함께 높다",
      points: [
        {
          label: "기록 범위",
          text: "오늘은 저녁만 알려져 있어 하루 전체 섭취량으로 판정하지 않는다. 기록된 저녁만 약 1,000~1,900kcal, 단백질 55~125g으로 추정하며 실제 치킨 섭취 조각 수가 오차를 가장 크게 만든다."
        },
        {
          label: "단백질",
          text: "치킨 덕분에 저녁 한 끼의 단백질은 충분했을 가능성이 있다. 다만 튀김옷과 껍질·양념이 함께 들어가므로 상체 근육 목표를 위한 일상적인 단백질 공급원으로 매번 선택하기보다는 다음 끼니를 두부·달걀·생선·살코기처럼 담백하게 구성한다."
        },
        {
          label: "당·나트륨",
          text: "일반 콜라 한 잔을 250ml로 보면 당류 약 27g이고, 양념치킨 소스까지 합친 유리당은 약 40~80g으로 추정된다. 나트륨도 약 1,800~4,000mg으로 하루 참고 상한에 닿거나 넘었을 가능성이 있다."
        },
        {
          label: "균형",
          text: "식이섬유는 약 1~4g으로 적다. 오늘 추가로 먹는다면 과자나 달달한 음료보다 물과 채소·통과일을 선택하는 편이 균형을 맞추기 쉽다."
        }
      ]
    },
    actions: [
      "오늘 남은 시간에는 물을 충분히 마시고 추가 야식과 단 음료는 건너뛴다.",
      "다음 끼니는 튀김·가공육보다 담백한 단백질과 채소를 먼저 고른다.",
      "다음 기록에서 오늘의 다른 끼니와 치킨을 대략 몇 조각 먹었는지 알려주면 범위를 좁힌다."
    ]
  },
  {
    date: "2026-08-21",
