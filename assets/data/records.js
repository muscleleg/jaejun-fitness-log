...
window.healthRecordData = [
  {
    ...
    meals: [
      ...
      { label: "간식·보충제", items: ["로투스 비스코프 2조각", "미니 꿀꽈배기 과자 1조각", "미니 약과 1조각", "레몬 사탕 2개", "다이아몬드푸드 망고 크림크래커 1개", "오쏘몰 비타민 1회분 (제품 종류 미확인)"] },
      { label: "섭취 예정 · 현재 합계 제외", items: ["오리온 치킨팝 65g 1봉지 (맛 미확인·먹은 뒤 확인 필요)"] }
    ],
    nutritionEstimate: {
      calories: "현재까지 약 1,670~2,470 kcal · 치킨팝 섭취 시 약 2,000~2,810 kcal",
      protein: "현재까지 약 81~112g · 치킨팝 섭취 시 약 83~114g",
      freeSugar: "현재 약 47~77g · 치킨팝 닭강정맛 섭취 시 약 65~95g",
      nutrients: {
        energy: { min: 1670, max: 2470, unit: "kcal" },
        protein: { min: 81, max: 112, unit: "g" },
        carbohydrate: { min: 230, max: 350, unit: "g" },
        totalFat: { min: 48, max: 90, unit: "g" },
        saturatedFat: { min: 12, max: 28, unit: "g" },
        freeSugar: { min: 47, max: 77, unit: "g" },
        fiber: { min: 6, max: 14, unit: "g" },
        sodium: { min: 2200, max: 4500, unit: "mg" }
      },
      assumptions: "기존 기록에 저녁과 추가 간식을 합산했다. 흰밥은 보통 한 공기, 소고기 볶음은 약 120~180g, 연두부는 간장 소스를 포함한 보통 1회분으로 가정했다. 더단백 초코는 250ml 105kcal·단백질 20g, 야채농장은 오늘 총 2팩으로 170kcal·단백질 4g·당류 24g(사용자 확인: 팩당 12g)을 반영했다. 로투스는 오늘 총 2조각, 레몬 사탕은 총 2개, 망고 크림크래커는 1개 약 40~45kcal·단백질 1g 미만으로 추정했다. 유리당은 주스·사탕·과자·약과와 볶음 양념에 든 당을 합쳐 범위로 추정했다. 치킨팝은 아직 먹지 않은 65g 한 봉지로 보고 현재 합계에서 제외했으며, 맛이 확인되지 않아 열량 예상에는 공식 제품별 약 333~338kcal와 단백질 약 2g을 더했다. 유리당 예상은 닭강정맛의 당류 18g을 기준으로 했다. 오쏘몰은 제품 종류에 따라 열량과 당류 차이가 있어 넓은 범위로 반영했다.",
    ...
    advice: {
      ...
      points: [
        ...
        { label: "영양", text: "치킨팝을 제외한 현재까지 약 1,670~2,470kcal와 단백질 81~112g으로 추정한다. 치킨팝 한 봉지를 먹으면 약 2,000~2,810kcal와 단백질 83~114g이 된다. 단백질 목표 105g 충족 여부는 소고기와 연두부 양에 크게 좌우된다." },
        { label: "유리당", text: "현재 약 47~77g으로 추정된다. 남성 30~49세 참고 에너지 2,500kcal에 적용한 WHO 상한 약 63g의 75~122%로, 추정 범위 상단에서는 초과한다. 추가 이점 기준 약 31g은 확실히 넘는다. 야채농장 2팩의 당류 24g과 사탕·비스킷·약과가 주된 원인이다." },
        { label: "영양 균형", text: "단백질은 목표 105g의 77~107%로 충족 가능하지만, 식이섬유는 30g 목표의 20~47%로 부족하다. 나트륨은 WHO 상한 2,000mg의 110~225%로 초과 가능성이 높고, 탄수화물은 권고 하한 313g의 73~112%라 실제 식사량에 따라 부족할 수 있다." },
